// backends/iluvatar/device/iluvatar_device.cpp
// IXUCA device HAL — uses CoreX CUDA-compatible runtime API.
// 天数智芯 Iluvatar BI-V150S 通过 CoreX SDK 提供标准 CUDA API 兼容层
// (cudaMalloc, cudaMemcpy, cudaStreamCreate 等均在 /usr/local/corex/include/ 中实现)
...
extern "C" void iluvatar_set_last_error_ext(const char *msg) {
  gpu_last_error_str = msg ? msg : "";
}

extern "C" const char *iluvatar_get_last_error_ext(void) {
...
static C_Status ix_initialize(void) {
  ...
  if (err != cudaSuccess) {
    iluvatar_set_last_error_ext(cudaGetErrorString(err));
    return C_FAILED;
  }
  return C_SUCCESS;
}
...
static C_Status ix_deinit_device(C_Device device) {
  return C_SUCCESS;
}
...
static C_Status ix_get_device_list(size_t *devices) {
  ...
  for (size_t i = 0; i < count; ++i) devices[i] = i;
  return C_SUCCESS;
}
...
static C_Status ix_device_memory_allocate(C_Device device, void **ptr,
  ...
  if (size == 0) { *ptr = nullptr; return C_SUCCESS; }
...
static C_Status ix_memory_copy_h2d(C_Device device, void *dst,
...
static C_Status ix_memory_copy_d2h(C_Device device, void *dst,
                                   const void *src, size_t size) {
  if (!dst || !src) return C_FAILED;
...
static C_Status ix_memory_copy_d2d(C_Device device, void *dst,
                                   const void *src, size_t size) {
  if (!dst || !src) return C_FAILED;
...
static C_Status ix_async_memory_copy_p2p(C_Device dst_device,
                                         ...
                                         size_t size) {
  if (!dst || !src) return C_FAILED;
  cudaError_t err = cudaMemcpyPeerAsync(dst, dst_device->id, src,
                                        src_device->id, size,
                                        reinterpret_cast<cudaStream_t>(stream));
  if (err != cudaSuccess) {
    iluvatar_set_last_error_ext(cudaGetErrorString(err));
...
static C_Status ix_stream_wait_event(C_Device device, C_Stream stream,
                                    C_Event event) {
  cudaError_t err = cudaStreamWaitEvent(reinterpret_cast<cudaStream_t>(stream),
...
static C_Status ix_get_compute_capability(C_Device device, size_t *capability) {
  ...
  if (err != cudaSuccess) { iluvatar_set_last_error_ext(cudaGetErrorString(err)); return C_FAILED; }
...
static C_Status ix_get_device_name(C_Device device, char *buf, size_t buf_size) {
...
struct IluvatarProfiler {
  ...
  bool in_event = false;
  bool running = false;
  std::unordered_map<std::string, IluvatarProfilerEvent> events;
  std::vector<C_ProfilerEvent> report_cache;
};

static C_Status ix_profiler_create(C_Profiler *prof, const char *name,
                                   int device_id) {
  if (!prof) return C_FAILED;
  auto *p = new (std::nothrow) IluvatarProfiler();
  if (!p) return C_FAILED;
  if (name) p->name = name;
...
static C_Status ix_profiler_destroy(C_Profiler prof) {
  if (!prof) return C_FAILED;
  delete reinterpret_cast<IluvatarProfiler *>(prof);
  return C_SUCCESS;
}

static C_Status ix_profiler_start(C_Profiler prof) {
  if (!prof) return C_FAILED;
  reinterpret_cast<IluvatarProfiler *>(prof)->running = true;
  return C_SUCCESS;
}

static C_Status ix_profiler_stop(C_Profiler prof) {
  if (!prof) return C_FAILED;
  reinterpret_cast<IluvatarProfiler *>(prof)->running = false;
  return C_SUCCESS;
}

static C_Status ix_profiler_reset(C_Profiler prof) {
  if (!prof) return C_FAILED;
  auto *p = reinterpret_cast<IluvatarProfiler *>(prof);
  p->events.clear();
...
static C_Status ix_profiler_begin_event(C_Profiler prof, const char *name) {
  if (!prof || !name) return C_FAILED;
  auto *p = reinterpret_cast<IluvatarProfiler *>(prof);
  if (!p->running) return C_SUCCESS;
...
static C_Status ix_profiler_end_event(C_Profiler prof) {
  if (!prof) return C_FAILED;
  auto *p = reinterpret_cast<IluvatarProfiler *>(prof);
  if (!p->running || !p->in_event) return C_SUCCESS;
  auto end = std::chrono::high_resolution_clock::now();
  double ms = std::chrono::duration<double, std::milli>(end - p->event_start).count();
  p->in_event = false;
  auto &ev = p->events[p->current_event_name];
  ev.name = p->current_event_name;
  ev.calls++;
  ev.total_ms += ms;
  if (ms < ev.min_ms) ev.min_ms = ms;
  if (ms > ev.max_ms) ev.max_ms = ms;
  return C_SUCCESS;
}

static C_Status ix_profiler_get_events(C_Profiler prof, C_ProfilerEvent **events,
                                       size_t *count) {
  if (!prof || !events || !count) return C_FAILED;
  auto *p = reinterpret_cast<IluvatarProfiler *>(prof);
  for (auto &e : p->report_cache) {
    if (e.name) { free(const_cast<char *>(e.name)); e.name = nullptr; }
  }
  p->report_cache.clear();
...
INSIGHT_GPU_API C_Status InitPluginGPU(CustomRuntimeParams *params) {
  if (!params || !params->interface) return C_FAILED;
  ...
  // Error
  iface->get_last_error = iluvatar_get_last_error_ext;
  ...
  // Device identity
  params->device_type = const_cast<char *>("Iluvatar");
  params->sub_device_type = const_cast<char *>("v1.0");

  if (params->register_kernel) {
    iluvatar_sync_kernels(params->register_kernel);
  }
