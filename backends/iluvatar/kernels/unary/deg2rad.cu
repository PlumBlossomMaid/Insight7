// backends/cuda/kernels/unary/deg2rad.cu
/**
 * @file deg2rad.cu
 * @brief CUDA kernel for deg2rad operation.
 */
...
__global__ void deg2rad_f32_kernel(const float *x, float *out, int64_t n,
  ...
  if (idx < n) {
    ...
    out[off_out] = x[off_x] * 0.017453292519943295f;
  }
}

__global__ void deg2rad_f64_kernel(const double *x, double *out, int64_t n,
                                   int ndim, const int64_t *dims,
                                   const int64_t *x_strides,
                                   const int64_t *out_strides) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
    int64_t off_x = unary_offset(idx, ndim, dims, x_strides);
    int64_t off_out = unary_offset(idx, ndim, dims, out_strides);
    out[off_out] = x[off_x] * 0.017453292519943295;
  }
}

extern "C" {

C_Status deg2rad_kernel_gpu(void **inputs, void **outputs) {
  InsightArray *x = static_cast<InsightArray *>(inputs[0]);
  InsightArray *out = static_cast<InsightArray *>(outputs[0]);

  if (!x || !out) {
    gpu_set_last_error("deg2rad: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    ...
    gpu_set_last_error("deg2rad: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_ILUVATAR_KERNEL(deg2rad, INSIGHT_DTYPE_F32, deg2rad_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(deg2rad, INSIGHT_DTYPE_F64, deg2rad_kernel_gpu);
