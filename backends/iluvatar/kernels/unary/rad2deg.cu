// backends/cuda/kernels/unary/rad2deg.cu
/**
 * @file rad2deg.cu
 * @brief CUDA kernel for rad2deg operation.
 */
...
__global__ void rad2deg_f32_kernel(const float *x, float *out, int64_t n,
  ...
  if (idx < n) {
    ...
    out[off_out] = x[off_x] * 57.29577951308232f;
  }
}

__global__ void rad2deg_f64_kernel(const double *x, double *out, int64_t n,
                                   int ndim, const int64_t *dims,
                                   const int64_t *x_strides,
                                   const int64_t *out_strides) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
    int64_t off_x = unary_offset(idx, ndim, dims, x_strides);
    int64_t off_out = unary_offset(idx, ndim, dims, out_strides);
    out[off_out] = x[off_x] * 57.29577951308232;
  }
}

extern "C" {

C_Status rad2deg_kernel_gpu(void **inputs, void **outputs) {
  InsightArray *x = static_cast<InsightArray *>(inputs[0]);
  InsightArray *out = static_cast<InsightArray *>(outputs[0]);

  if (!x || !out) {
    gpu_set_last_error("rad2deg: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    ...
    gpu_set_last_error("rad2deg: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_ILUVATAR_KERNEL(rad2deg, INSIGHT_DTYPE_F32, rad2deg_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(rad2deg, INSIGHT_DTYPE_F64, rad2deg_kernel_gpu);
