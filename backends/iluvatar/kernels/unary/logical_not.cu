// backends/cuda/kernels/unary/logical_not.cu
/**
 * @file logical_not.cu
 * @brief CUDA kernel for logical NOT operation.
 *
 * Returns true if element is zero, false otherwise.
 */
...
template <typename T>
__global__ void logical_not_kernel(const T *x, bool *out, int64_t n, int ndim,
                                   const int64_t *dims,
  ...
  if (idx < n) {
    ...
    out[off_out] = (x[off_x] == 0);
  }
}

extern "C" {

C_Status logical_not_kernel_gpu(void **inputs, void **outputs) {
  InsightArray *x = static_cast<InsightArray *>(inputs[0]);
  InsightArray *out = static_cast<InsightArray *>(outputs[0]);

  if (!x || !out) {
    gpu_set_last_error("logical_not: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    ...
    gpu_set_last_error("logical_not: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_ILUVATAR_KERNEL(logical_not, INSIGHT_DTYPE_BOOL, logical_not_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(logical_not, INSIGHT_DTYPE_F32, logical_not_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(logical_not, INSIGHT_DTYPE_F64, logical_not_kernel_gpu);
