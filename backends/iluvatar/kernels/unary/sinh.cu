// backends/cuda/kernels/unary/sinh.cu
/**
 * @file sinh.cu
 * @brief CUDA kernel for sinh operation.
 */
...
__global__ void sinh_f32_kernel(const float *x, float *out, int64_t n, int ndim,
  ...
  if (idx < n) {
    ...
    out[off_out] = sinhf(x[off_x]);
  }
}

__global__ void sinh_f64_kernel(const double *x, double *out, int64_t n,
  ...
  if (idx < n) {
    int64_t off_x = unary_offset(idx, ndim, dims, x_strides);
    int64_t off_out = unary_offset(idx, ndim, dims, out_strides);
    out[off_out] = sinh(x[off_x]);
  }
}

extern "C" {

C_Status sinh_kernel_gpu(void **inputs, void **outputs) {
  InsightArray *x = static_cast<InsightArray *>(inputs[0]);
  InsightArray *out = static_cast<InsightArray *>(outputs[0]);

  if (!x || !out) {
    gpu_set_last_error("sinh: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    ...
    gpu_set_last_error("sinh: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_ILUVATAR_KERNEL(sinh, INSIGHT_DTYPE_F32, sinh_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(sinh, INSIGHT_DTYPE_F64, sinh_kernel_gpu);
