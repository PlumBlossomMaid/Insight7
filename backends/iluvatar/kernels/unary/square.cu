// backends/cuda/kernels/unary/square.cu
/**
 * @file square.cu
 * @brief CUDA kernel for square operation.
 */
...
__global__ void square_f32_kernel(const float *x, float *out, int64_t n,
  ...
  if (idx < n) {
    ...
    out[off_out] = x[off_x] * x[off_x];
  }
}

__global__ void square_f64_kernel(const double *x, double *out, int64_t n,
                                  int ndim, const int64_t *dims,
                                  const int64_t *x_strides,
                                  const int64_t *out_strides) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
...
C_Status square_kernel_gpu(void **inputs, void **outputs) {
  InsightArray *x = static_cast<InsightArray *>(inputs[0]);
  InsightArray *out = static_cast<InsightArray *>(outputs[0]);

  if (!x || !out) {
    gpu_set_last_error("square: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    ...
    gpu_set_last_error("square: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_ILUVATAR_KERNEL(square, INSIGHT_DTYPE_F32, square_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(square, INSIGHT_DTYPE_F64, square_kernel_gpu);
