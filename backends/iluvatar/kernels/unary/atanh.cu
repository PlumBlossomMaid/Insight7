// backends/cuda/kernels/unary/atanh.cu
/**
 * @file atanh.cu
 * @brief CUDA kernel for atanh operation.
 */
...
__global__ void atanh_f32_kernel(const float *x, float *out, int64_t n,
  ...
  if (idx < n) {
    ...
    out[off_out] = atanhf(x[off_x]);
  }
}

__global__ void atanh_f64_kernel(const double *x, double *out, int64_t n,
                                 int ndim, const int64_t *dims,
                                 const int64_t *x_strides,
                                 const int64_t *out_strides) {
  int64_t idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
    int64_t off_x = unary_offset(idx, ndim, dims, x_strides);
    int64_t off_out = unary_offset(idx, ndim, dims, out_strides);
    out[off_out] = atanh(x[off_x]);
  }
}

extern "C" {

C_Status atanh_kernel_gpu(void **inputs, void **outputs) {
  InsightArray *x = static_cast<InsightArray *>(inputs[0]);
  InsightArray *out = static_cast<InsightArray *>(outputs[0]);

  if (!x || !out) {
    gpu_set_last_error("atanh: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    ...
    gpu_set_last_error("atanh: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_ILUVATAR_KERNEL(atanh, INSIGHT_DTYPE_F32, atanh_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(atanh, INSIGHT_DTYPE_F64, atanh_kernel_gpu);
