// backends/cuda/kernels/unary/exp2.cu
/**
 * @file exp2.cu
 * @brief CUDA kernel for exp2 operation.
 */
...
__global__ void exp2_f32_kernel(const float *x, float *out, int64_t n, int ndim,
  ...
  if (idx < n) {
    ...
    out[off_out] = exp2f(x[off_x]);
  }
}

__global__ void exp2_f64_kernel(const double *x, double *out, int64_t n,
  ...
  if (idx < n) {
    int64_t off_x = unary_offset(idx, ndim, dims, x_strides);
    int64_t off_out = unary_offset(idx, ndim, dims, out_strides);
    out[off_out] = exp2(x[off_x]);
  }
}

extern "C" {

C_Status exp2_kernel_gpu(void **inputs, void **outputs) {
  InsightArray *x = static_cast<InsightArray *>(inputs[0]);
  InsightArray *out = static_cast<InsightArray *>(outputs[0]);

  if (!x || !out) {
    gpu_set_last_error("exp2: null array pointer");
    return C_FAILED;
  }
  ...
  default:
    ...
    gpu_set_last_error("exp2: unsupported dtype");
    return C_FAILED;
  }
...
REGISTER_ILUVATAR_KERNEL(exp2, INSIGHT_DTYPE_F32, exp2_kernel_gpu);
REGISTER_ILUVATAR_KERNEL(exp2, INSIGHT_DTYPE_F64, exp2_kernel_gpu);
