...
void expect_eye_2d(const Array &gpu_arr, int rows, int cols, int offset) {
  Array cpu_arr = gpu_arr.to(CPUPlace());
  ASSERT_EQ(cpu_arr.shape().ndim(), 2);
  ASSERT_EQ(cpu_arr.shape().dim(0), rows);
  ASSERT_EQ(cpu_arr.shape().dim(1), cols);
...
TEST_F(CreationTestGPU, FullFloat64) {
  Array a = full({2, 2}, 2.718281828, DType::F64, GPUPlace(0));
...
TEST_F(CreationTestGPU, EyeNegativeOffset) {
  Array a = eye(3, 3, -1, DType::F32, GPUPlace(0));
  EXPECT_EQ(a.place(), GPUPlace(0));
...
TEST_F(CreationTestGPU, LinspaceNegativeRange) {
  Array a = linspace(-1, 1, 5, DType::F32, GPUPlace(0));
  EXPECT_EQ(a.place(), GPUPlace(0));
