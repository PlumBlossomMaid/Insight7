...
TEST_F(SignalTestGPU, UnwrapBasic) {
  std::vector<double> data = {0.0,          M_PI_2,     M_PI,
                              3.0 * M_PI_2, 2.0 * M_PI, 3.0 * M_PI};
...
TEST_F(SignalTestGPU, UnwrapScalar) {
  ...
  Array u = unwrap(a);
  Array u_cpu = u.to(CPUPlace());
  EXPECT_NEAR(u_cpu.item<double>(), M_PI, 1e-6);
}

TEST_F(SignalTestGPU, Unwrap2DAxis1) {
  std::vector<double> data = {0.0, M_PI_2, M_PI, 3.0 * M_PI_2,
                              0.1, M_PI_2, 3.2,  3.3};
